  ...
  if ($_SERVER["REQUEST_METHOD"]=="POST") {
	...
	if($query->rowCount()==0){
		...
		$result=$query->execute(array(
			...
			"Email"=>$_POST["email"],	
			"Password"=>$password,	
			"UserGroup"=>"Financial Institute",	
			"Option1"=>$_POST["financialInstitute1"],	
			"Active"=>"0"
		));
...
<div class="login-box">
  <div class="login-logo">
	...
	<br/><span class="fontsize16px">(เจ้าหน้าที่/พนักงาน สถาบันการเงิน)</span>
  </div>
    ...
    <form id="frm" name="frm" role="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	  ...
	  <div class="form-group has-feedback">
	    <select id='financialInstitute1' name='financialInstitute1' class='form-control select2' style='width: 100%;'>
		  <option value='0' selected>เลือกสถาบันการเงินที่สังกัด</option>
		  <?php
...
<script>
  $(document).ready(function () {
	...
	$.validator.addMethod("financialInstitute", function(value, element) {
      if($('#financialInstitute1').children("option:selected").val()=="0") {
        return false;
      }else{
	  	return true;
	  }
    }, "กรุณาระบุข้อมูลสถานบันการเงินที่สังกัด");
