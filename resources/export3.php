<?php
  ...
  $comm="SELECT * FROM transaction WHERE `Print`=1 AND `PrintDate`>CONCAT(DATE_ADD(CURDATE(), INTERVAL -1 DAY), ' 20:00:00') AND `PrintDate`<=CONCAT(CURRENT_DATE, ' 20:00:00')  
ORDER BY `PrintDate` ASC";
  ...
  if($query->rowCount()>0){
	$delimiter = ",";
	//transection (print) 2020-05-04 08.30-16.30
	$today=date('Y-m-d'); 
	$yesterday= date("Y-m-d", strtotime("-1 day",strtotime($today)));
    $filename = "transection (print) ".$yesterday." 20.00 - " . date('Y-m-d') . " 20.00.csv";
