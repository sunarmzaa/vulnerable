<?php
  ...
  //$comm="SELECT * FROM transaction ORDER BY `PrintDate` ASC"; //ลงทะเบียนสะสม ตั้งแต่วันที่ 2 พ.ค. 12.00 น. - วันที่ 3 พ.ค. 16.30 น.
  $comm="SELECT * FROM transaction WHERE `Print`=1 AND `PrintDate`<=CONCAT(CURRENT_DATE, ' 20:00:00') ORDER BY `PrintDate` ASC;"; //จัดพิมพ์เอกสารสะสม ตั้งแต่วันที่ 2 พ.ค. 12.00 น. - วันที่ 3 พ.ค. 16.30 น.
  $query=$conn->prepare($comm); 
  $query->execute();
  if($query->rowCount()>0){
	$delimiter = ",";
	//transection (total-print) 2020-05-02 12.00 - 2020-05-05 18.10
    $filename = "transection (total-print) 2020-05-02 12.00 - " . date('Y-m-d') . " 20.00.csv";
