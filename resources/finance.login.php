<?php
  ...
  if ($_SERVER["REQUEST_METHOD"]=="POST") {
	//lekstores
	//$password = encode($_POST["password1"], $_POST["email1"]);
	...
	if($query->rowCount()>0){
	  $row=$query->fetch();
	  $_SESSION["AUTHEN"]["USER_ID"]=$row["UserID"];
	  $_SESSION["AUTHEN"]["EMAIL"]=$row["Email"];
	  $_SESSION["AUTHEN"]["FULLNAME"]=$row["FullName"];
	  $_SESSION["AUTHEN"]["GROUP"]=$row["UserGroup"];
	  $_SESSION["AUTHEN"]["OPTION1"]=$row["Option1"];
	  $_SESSION["AUTHEN"]["OPTION2"]=$row["Option2"];
	  
	  if($_SESSION["AUTHEN"]["GROUP"]=="Administrator"){
		header("Location: finance.dashboard.php");
	    die();
	  }else if($_SESSION["AUTHEN"]["GROUP"]=="Financial Institute"){
		header("Location: finance.management.php");
	    die();
	  }else if($_SESSION["AUTHEN"]["GROUP"]=="Business Agent"){
		header("Location: finance.agent.management.php");
	    die();
	  }
...
<div class="login-box">
  ...
  <div class="login-box-body">
    ...
    <form id="frm" name="frm" role="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
      ...
      <div class="row">
        <div class="col-xs-8">
		  <!--<a href="finance.register.php"> สมัครสมาชิก (สถาบันการเงินฯ)</a><br/>-->
		  <a href="finance.agent.register.php"> สมัครสมาชิก (ตัวแทนธุรกิจ)</a>
          <!--
