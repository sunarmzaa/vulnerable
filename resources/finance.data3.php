<?php
  ...
  if(isset($_GET["startdate"]) && isset($_GET["stopdate"])){
	if($_GET["startdate"]!="" && $_GET["stopdate"]!=""){
	  ...
	  $comm="SELECT * FROM transection WHERE CreateDate>'".$startdate." 00:00:00' AND CreateDate<='".$stopdate." 23.59.59' AND BusinessType='".$business."'";
	  ...
	  if($query->rowCount()>0){
		...
		for($i=0; $i<$query->rowCount(); $i++){
		  ...
		  $result["Action"]="<a href='"."finance.agent.trace.php?id=".$rows[$i]["TransectionID"]."' class='fa fa-file'></a>"; //$rows[$i]["TransectionID"]
